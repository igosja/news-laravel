...
@section('content')
    <h1 class="text-center">{{ __('messages.Posts') }}</h1>
    ...
    <div class="row">
        <div id="w0" class="col-lg-12 col-md-12 col-sm-12 col-xs-12 table-responsive">
            ...
            <table class="table table-bordered table-hover">
                    ...
                    <th class="col-lg-1">
                        <a
                                ...
                                @elseif ('-id' === app('request')->query('sort'))
                                    desc
                                @endif"
                                href="{{ route('post.index', ['sort' => ('id' === app('request')->query('sort') ? '-id' : 'id')]) }}"
                        >
                            ID
                        </a>
                    </th>
                    <th>
                        <a
                                ...
                                @elseif ('-name' === app('request')->query('sort'))
                                    desc
                                @endif"
                                href="{{ route('post.index', ['sort' => ('name' === app('request')->query('sort') ? '-name' : 'name')]) }}"
                        >
                            Name
                        </a>
                    </th>
                    <th class="col-lg-3">
                        <a
                                ...
                                @elseif ('-is_active' === app('request')->query('sort'))
                                    desc
                                @endif"
                                href="{{ route('post.index', ['sort' => ('is_active' === app('request')->query('sort') ? '-is_active' : 'is_active')]) }}"
                        >
                            Is active
                        </a>
                    </th>
                ...
                <tr class="filters" data-url="{{ route('post.index') }}">
                    <td>
                ...
                @foreach ($posts as $post)
                    <tr data-key="{{ $post->id }}">
                        ...
                        <td class="text-center">
                            <a href="{{ route('post.show', ['post' => $post]) }}" title="Переглянути"
                            ...
                            <a href="{{ route('post.edit', ['post' => $post]) }}" title="Оновити"
                            ...
                            <form
                                    action="{{ route('post.destroy', ['post' => $post]) }}" method="POST">
                                @csrf
                                @method('DELETE')
            ...
            <nav>
                <ul class="pagination">
                    <li class="page-item prev @if($posts->currentPage() - 1 < 1) disabled @endif">
                        <a class="page-link"
                           href="{{ route(Route::currentRouteName(), ['page' => $posts->currentPage() - 1]) }}">
                    ...
                    @for ($i = $posts->currentPage() - 2; $i < $posts->currentPage() + 2; $i++)
                        ...
                        @endif
                    @endfor
                    <li class="page-item next @if($posts->currentPage() + 1 > $posts->lastPage()) disabled @endif">
                        <a class="page-link"
                           href="{{ route(Route::currentRouteName(), ['page' => $posts->currentPage() + 1]) }}">
                            <span aria-hidden="true">»</span>
                        </a>
